...
void Mass_Storage_In(void) //Mass Storage IN transfer
{
	switch (Bot_State)
	{
		case BOT_CSW_Send:
...
void Mass_Storage_Out(void) //Mass Storage OUT transfer
{
	u8 CMD;
	CMD = CBW.CB[0];
...
void CBW_Decode(void) //Decode the received CBW and call the related SCSI command  routine
{
	u32 counter;
 ...
 *                  u16 Data_Length : the number of Bytes to transfer.
 *******************************************************************************/
void Transfer_Data_Request(u8* dataPointer, u16 dataLength)
{
	USB_SIL_Write(EP1_IN, dataPointer, dataLength);
 ...
 * Return         : None.
 *******************************************************************************/
void Set_CSW(u8 CSW_Status, u8 sendPermission)
{
